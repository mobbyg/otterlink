    ...
    : QWidget(parent), m_client(client)
{
    ...
    connect(m_calendar,&QCalendarWidget::clicked,this,&OtterEventsWidget::calendarDateChanged);
    ...
    connect(m_list,&QListWidget::itemSelectionChanged,this,[this]{
        ...
        m_editButton->setEnabled(mine);
        m_deleteButton->setEnabled(mine);
    });
    ...
    m_year=now.year();
    m_month=now.month();
    m_calendar->setSelectedDate(QDate(m_year,m_month,now.day()));
    m_client->loadChatChannels();
    loadCurrentMonth();
}

void OtterEventsWidget::loadCurrentMonth()
{
    m_monthLabel->setText(QDate(m_year,m_month,1).toString(QStringLiteral("MMMM yyyy")));
    const int day=qMin(m_calendar->selectedDate().day(),QDate(m_year,m_month,1).daysInMonth());
    m_calendar->setSelectedDate(QDate(m_year,m_month,day));
    m_client->loadEvents(m_year,m_month);
}

void OtterEventsWidget::previousMonth()
{
    if(--m_month<1){
        m_month=12;
        --m_year;
...
void OtterEventsWidget::nextMonth()
{
    if(++m_month>12){
        m_month=1;
        ++m_year;
...
void OtterEventsWidget::calendarDateChanged(const QDate &date)
{
    ...
    for(int i=0;i<m_events.size();++i){
        const auto event=m_events.at(i).toObject();
        QDate eventDate;
        if(event.value(QStringLiteral("all_day")).toBool())
            eventDate=QDate::fromString(event.value(QStringLiteral("start_at")).toString(),QStringLiteral("yyyy-MM-dd"));
        else
            eventDate=QDateTime::fromString(event.value(QStringLiteral("start_at")).toString(),Qt::ISODate).toLocalTime().date();
...
void OtterEventsWidget::eventsLoaded(const QJsonArray &events,int year,int month)
{
    ...
    const int day=qMin(m_calendar->selectedDate().day(),QDate(year,month,1).daysInMonth());
...
void OtterEventsWidget::deleteSelected()
{
    ...
    if(e.isEmpty())return;
    if(QMessageBox::question(this,QStringLiteral("Delete Event"),QStringLiteral("Delete '%1'?").arg(e.value("title").toString()))==QMessageBox::Yes)
        m_client->deleteEvent(e.value("id").toVariant().toLongLong());
}

void OtterEventsWidget::openEditor(const QJsonObject &event,bool editing)
{
    QDialog dialog(this);
    dialog.setWindowTitle(editing?QStringLiteral("Edit Event"):QStringLiteral("Add Event"));
    ...
    type->addItem(QStringLiteral("Public"),"public");
    type->addItem(QStringLiteral("Community"),"community");

    auto *target=new QComboBox(&dialog);
    target->addItem(QStringLiteral("No community"),0);
    for(const auto &value:m_channels){
        const auto c=value.toObject();
        target->addItem(c.value("name").toString(),c.value("id").toVariant().toLongLong());
    }

    auto *allDay=new QCheckBox(QStringLiteral("All day"),&dialog);
    auto *start=new QDateTimeEdit(QDateTime::currentDateTime(),&dialog);
    start->setCalendarPopup(true);
    auto *end=new QDateTimeEdit(QDateTime::currentDateTime().addSecs(3600),&dialog);
    ...
    form->addRow(QStringLiteral("Community:"),target);
    form->addRow(allDay);
    form->addRow(QStringLiteral("Start:"),start);
    form->addRow(QStringLiteral("End:"),end);
    ...
    if(editing){
        title->setText(event.value("title").toString());
        description->setPlainText(event.value("description").toString());
        type->setCurrentIndex(event.value("event_type").toString()=="community"?1:0);

        const qint64 targetId=event.value("target_id").toVariant().toLongLong();
        const int idx=target->findData(targetId);
        if(idx>=0)target->setCurrentIndex(idx);

        const bool day=event.value("all_day").toBool();
        allDay->setChecked(day);
        if(day){
            start->setDate(QDate::fromString(event.value("start_at").toString(),"yyyy-MM-dd"));
            end->setDate(QDate::fromString(event.value("end_at").toString(),"yyyy-MM-dd"));
        }else{
            start->setDateTime(QDateTime::fromString(event.value("start_at").toString(),Qt::ISODate).toLocalTime());
            end->setDateTime(QDateTime::fromString(event.value("end_at").toString(),Qt::ISODate).toLocalTime());
        }
    }
    ...
    QJsonObject payload{
        ...
        {"event_type",type->currentData().toString()},
        {"target_type",target->currentData().toLongLong()>0?"chat":"none"},
        {"target_id",target->currentData().toLongLong()},
        {"all_day",allDay->isChecked()}
    };

    if(allDay){
        payload["start_at"]=start->date().toString("yyyy-MM-dd");
        payload["end_at"]=end->date().toString("yyyy-MM-dd");
    }else{
    ...
    if(editing)
        m_client->updateEvent(event.value("id").toVariant().toLongLong(),payload);
    else
        m_client->createEvent(payload);
}
