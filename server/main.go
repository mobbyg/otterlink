...
func main() {
	...
	if adminUsername := strings.TrimSpace(os.Getenv("OTTERLINK_ADMIN_USERNAME")); adminUsername != "" {
		found, err := accountService.EnsureAdmin(adminUsername)
		if err != nil {
			log.Fatalf("configure admin account: %v", err)
		}
		if !found {
			log.Printf("OTTERLINK_ADMIN_USERNAME=%q does not match an existing account", adminUsername)
		} else {
			log.Printf("web administration enabled for account %q", adminUsername)
		}
	}
	...
	protocolHandler := &protocol.DefaultHandler{
		Accounts:         accountService,
		Buddies:          buddyService,
		Presence:         presenceService,
		Events:           protocolServer.Broadcast,
		SendToConnection: protocolServer.SendToConnection,
	}
	protocolServer.Handler = protocolHandler
	protocolErr := make(chan error, 1)
	go func() { protocolErr <- protocolServer.ListenAndServe(ctx) }()

	oscarServer := &oscar.Server{
		Addr:          oscarAddr,
		Logger:        log.Default(),
	...
	case err := <-oscarErr:
		if err != nil {
			log.Fatalf("OSCAR server stopped: %v", err)
		}
	case <-ctx.Done():
	...
	case err := <-oscarErr:
		if err != nil {
			log.Printf("OSCAR shutdown: %v", err)
		}
	case <-shutdownCtx.Done():
...
func healthHandler(w http.ResponseWriter, _ *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(healthResponse{Status: "ok", Service: "otter-link"})
}
