...
func (s *Server) adminChatChannelRole(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := s.Chat.Role(id, target.ID); err != nil {
		if _, _, _, _, err := s.Chat.Join(id, chatUser(target)); err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
	}
	if req.Role == "mod" {
		if err := s.Chat.AdminSetRole(id, target.ID, "mod"); err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
	} else if req.Role == "op" {
		if err := s.Chat.AdminSetRole(id, target.ID, "op"); err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
	} else {
		if err := s.Chat.AdminSetRole(id, target.ID, "user"); err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
	}
	...
	for _, channel := range channels {
		if channel.ID == id {
			if !channel.Permanent {
				return errors.New("administrator role management is limited to permanent channels")
			}
			return nil
		}
	}
