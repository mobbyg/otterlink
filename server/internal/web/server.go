...
func (s *Server) index(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { http.Error(w, "web client unavailable", http.StatusInternalServerError); return }
...
func (s *Server) presenceList(w http.ResponseWriter, r *http.Request) {
	...
	if s.Presence == nil { http.Error(w, "presence unavailable", http.StatusInternalServerError); return }
...
func (s *Server) buddyList(w http.ResponseWriter, r *http.Request) {
	...
	list, err := s.Buddies.List(user.ID); if err != nil { http.Error(w, "unable to list buddies", http.StatusInternalServerError); return }
...
func (s *Server) chatList(w http.ResponseWriter, r *http.Request) {
	if _, ok := s.user(r); !ok { http.Error(w, "unauthorized", http.StatusUnauthorized); return }
	if s.Chat == nil { http.Error(w, "chat unavailable", http.StatusInternalServerError); return }
...
func (s *Server) chatSend(w http.ResponseWriter, r *http.Request) {
	...
	if message == "" { http.Error(w, "message is required", http.StatusBadRequest); return }
	if len([]rune(message)) > 2000 { http.Error(w, "message exceeds 2000 characters", http.StatusBadRequest); return }
...
func decodeJSON(w http.ResponseWriter, r *http.Request, dst any) bool {
	...
	if err := json.NewDecoder(r.Body).Decode(dst); err != nil { http.Error(w, "invalid JSON", http.StatusBadRequest); return false }
	return true
}
