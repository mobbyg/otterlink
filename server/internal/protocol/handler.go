...
func (h *DefaultHandler) handleSession(ctx context.Context, msg Message) Message {
	...
	case "whoami":
		...
		if err != nil { return ErrorResponse(msg.ID, ErrUnauthorized, err.Error()) }
		return SuccessResponse(msg.ID, ServiceSession, "whoami", user)
	case "logout":
		token, err := tokenFromPayload(msg.Payload)
		if err != nil { return ErrorResponse(msg.ID, ErrUnauthorized, "valid session token required") }
		user, err := h.Accounts.FromToken(token)
		if err != nil { return ErrorResponse(msg.ID, ErrUnauthorized, "invalid session") }
...
func (h *DefaultHandler) handlePresence(msg Message) Message {
	...
	case "get":
		...
		if !ok { return SuccessResponse(msg.ID, ServicePresence, "get", map[string]interface{}{"online": false}) }
...
func (h *DefaultHandler) markOnline(ctx context.Context, user accounts.User) {
	id := connectionID(ctx)
	if id == 0 || h.Presence == nil { return }
	h.mu.Lock()
	if h.connections == nil { h.connections = make(map[uint64]int64) }
	if old, ok := h.connections[id]; ok && old != user.ID { h.mu.Unlock(); h.markOffline(ctx, old); h.mu.Lock() }
	h.connections[id] = user.ID
	h.mu.Unlock()
	entry, becameOnline := h.Presence.OnlineConnection(user, id)
	if becameOnline && h.Events != nil { h.Events(Message{Type: TypeEvent, Service: ServicePresence, Action: "online", Payload: entry}) }
}
...
func (h *DefaultHandler) markOffline(ctx context.Context, userID int64) {
	...
	if id == 0 || h.Presence == nil { return }
	h.mu.Lock()
	if h.connections != nil { delete(h.connections, id) }
	h.mu.Unlock()
	entry, becameOffline := h.Presence.OfflineConnection(userID, id)
	if becameOffline && h.Events != nil { h.Events(Message{Type: TypeEvent, Service: ServicePresence, Action: "offline", Payload: entry}) }
}
...
func (h *DefaultHandler) OnDisconnect(ctx context.Context) {
	...
	if id == 0 { return }
	h.mu.Lock()
	userID, ok := h.connections[id]
	h.mu.Unlock()
	if ok { h.markOffline(ctx, userID) }
}

func (h *DefaultHandler) authenticate(msg Message) (accounts.User, error) {
	token, err := tokenFromPayload(msg.Payload)
	if err != nil { return accounts.User{}, err }
...
func tokenFromPayload(payload interface{}) (string, error) {
	...
	if err := decodePayload(payload, &input); err != nil { return "", err }
	input.Token = strings.TrimSpace(input.Token)
	if input.Token == "" { return "", errors.New("missing session token") }
