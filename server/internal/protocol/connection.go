...
// Connection implements the development line-oriented protocol. Each message
// is one JSON object terminated by a newline. The logical envelope is shared
// with future transports, so clients do not depend on TCP framing details.
...
func (c *Connection) ReadMessage() (Message, error) {
	...
	if len(line) > 64*1024 {
		return msg, fmt.Errorf("protocol message exceeds 64 KiB")
	}
	if err := json.Unmarshal(line, &msg); err != nil {
		return msg, fmt.Errorf("invalid protocol message: %w", err)
	}
	return msg, nil
}
