...
func (a AuthAPI) Login(w http.ResponseWriter, r *http.Request) {
	...
	if a.Presence != nil { a.Presence.OnlineHTTP(user, tokenConnectionID(token)) }
...
func (a AuthAPI) Logout(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { http.Error(w, "invalid session", http.StatusUnauthorized); return }
	if a.Chat != nil {
	...
	if err := a.Accounts.Logout(token); err != nil { http.Error(w, "logout failed", http.StatusInternalServerError); return }
	if a.Presence != nil { a.Presence.OfflineHTTP(user.ID, tokenConnectionID(token)) }
...
func bearerToken(r *http.Request) string {
	...
	if !strings.HasPrefix(value, "Bearer ") { return "" }
...
func decodeJSON(w http.ResponseWriter, r *http.Request, dst any) bool {
	if r.Header.Get("Content-Type") != "application/json" { http.Error(w, "content type must be application/json", http.StatusUnsupportedMediaType); return false }
