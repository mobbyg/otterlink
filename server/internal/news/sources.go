...
func TestValidateSource(t *testing.T) {
	...
	for _, tc := range valid { if err := validateSource(tc.name, tc.rawURL, tc.category, tc.refresh); err != nil { t.Fatalf("valid source rejected: %v", err) } }
	...
	for _, tc := range invalid { if err := validateSource(tc.name, tc.rawURL, tc.category, tc.refresh); err == nil { t.Fatalf("invalid source accepted: %+v", tc) } }
}
