...
func testDB(t *testing.T) *sql.DB {
	...
	_, err = db.Exec(`
		...
		CREATE TABLE chat_channels (
			...
			original_mod_user_id INTEGER,
			original_mod_username TEXT,
			permanent INTEGER,
			allow_ops_to_create_ops INTEGER
		);
		...
		CREATE TABLE chat_channel_bans (
			channel_id INTEGER,
			user_id INTEGER,
			PRIMARY KEY(channel_id, user_id)
		);
...
func TestOriginalModCanCreateModsButDelegatedModCannot(t *testing.T) {
	...
	if _, _, _, _, err = h.Join(channel.ID, carol); err != nil {
		t.Fatal(err)
	}
	if err := h.SetRole(channel.ID, alice.ID, bob.ID, "mod"); err != nil {
		t.Fatal(err)
	}
	if err := h.SetRole(channel.ID, bob.ID, carol.ID, "mod"); err == nil {
		t.Fatal("delegated mod was allowed to create a mod")
	}
}
...
func TestOperatorCreationRequiresChannelFlag(t *testing.T) {
	...
	if err := h.SetRole(channel.ID, alice.ID, bob.ID, "op"); err != nil {
		t.Fatal(err)
	}
	if err := h.SetRole(channel.ID, bob.ID, alice.ID, "op"); err == nil {
		t.Fatal("operator affected original mod")
	}
}
